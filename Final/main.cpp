...
GLfloat speed2 = 0.018f;//carwhite1T

//bluecar
GLfloat position3 = 0.0f;
GLfloat speed3 = 0.015f;//carBlue
...
GLfloat speed16 = 0.04f;

GLfloat position17 = 0.0f;
GLfloat speed17 = 0.03f;
...
glColor3ub(255, 255, 255);
     ...
     //car 5
    glPushMatrix();
glTranslatef(position16,0.0f, 0.0f);
glBegin(GL_QUADS);
     glColor3f(0,0,1);
     glVertex2f(.43,-0.88);
     glVertex2f(.43,-0.85);
...
//car 6
glPushMatrix();
glTranslatef(position17,0.0f, 0.0f);
     glBegin(GL_QUADS);
     glColor3ub(255,255,255);
     glVertex2f(.23,-0.95);
     glVertex2f(.23,-0.92);
...
/*glBegin(GL_QUADS);
	...
	glBegin(GL_TRIANGLE_FAN);
		glVertex2f(x, y); // center of circle
		for(X = 0; X <= triangleAmount;X++) {
			glVertex2f(
		            x + (radius * cos(X *  twicePi / triangleAmount)),
			    y + (radius * sin(X* twicePi / triangleAmount))
			);
		}
	glEnd();
...
glColor3ub(7, 8, 7);
	...
	glBegin(GL_TRIANGLE_FAN);
		glVertex2f(x, y); // center of circle
		for(Y = 0; Y <= triangleAmount;Y++) {
			glVertex2f(
		            x + (radius * cos(Y *  twicePi / triangleAmount)),
			    y + (radius * sin(Y* twicePi / triangleAmount))
			);
		}
	glEnd();
glPopMatrix();
